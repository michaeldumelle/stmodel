strnorm <- function(object, mu, size, ...){
  UseMethod("strnorm", object = object)
}

strnorm.matrix <- function(object, mu, size){
  n_st <- nrow(object)
...
strnorm.default <- function(object, mu, size, xcoord, ycoord = NULL, tcoord, data,
                            sp_cor, t_cor, chol, diag_tol, h_options = NULL){
  ...
  if (chol){
    ...
    st_covariance <- make_stcovariance(covparam_object = object, h_s_large = h_s_large,
                                                  ...
                                                  t_cor = t_cor)
    strnorm_sim <- strnorm.matrix(object = st_covariance, mu = mu, size = size)
  } else {
...
# }
# strnorm.default.productsum <- function(object, mu, size, xcoord, ycoord = NULL, tcoord, data,
#                            sp_cor, t_cor, chol, diag_tol, h_options){
#   if (chol){
#     if (is.null(ycoord)){
#       h_s_large <- make_h(coord1 = data[[xcoord]],
#                           distmetric = h_options$h_s_distmetric)
...
#       h_t_large <- make_h(coord1 = data[[tcoord]], distmetric = h_options$h_t_distmetric)
#     }
#     st_covariance <- make_stcovariance.productsum(covparam_object = object, h_s_large = h_s_large,
#                                         h_t_large = h_t_large, sp_cor = sp_cor,
#                                         t_cor = t_cor)
#     strnorm_sim <- strnorm.matrix(object = st_covariance, mu = mu, size = size)
#   } else {
...
#     n_st <- n_s * n_t
#
#     strnorm_sim <- vapply(1:size, function(x) {
#       strnorm_sim <- zs_chol_r_s_small %*% rnorm(n_s, sd = sqrt(covparam_object[["s_de"]])) +
#         multiply_z(mx = rnorm(n_s, sd = sqrt(covparam_object[["s_ie"]])), z_type = "spatial", n_s = n_s, n_t = n_t, side = "left") +
#         zs_chol_r_t_small %*% rnorm(n_t, sd = sqrt(covparam_object[["t_de"]])) +
#         multiply_z(mx = rnorm(n_t, sd = sqrt(covparam_object[["t_ie"]])), z_type = "temporal", n_s = n_s, n_t = n_t, side = "left") +
#         chol_r_st %*% rnorm(n_st, sd = sqrt(covparam_object[["st_de"]])) +
#         rnorm(n_st, sd = sqrt(covparam_object[["st_ie"]]))
#       strnorm_sim <- strnorm_sim[spint$ordered_data_dense$observed]
#       strnorm_sim <- strnorm_sim[order(spint$ordered_data_o$original_key)]
#       return(strnorm_sim)
#     }, double(n_st))
#   }
#   return(strnorm_sim)
