sv_fn <- function(stcov, ...){
  UseMethod("sv_fn", object = stcov)
}



sv_fn.productsum <- function(par, est_object, ...){
  # multiply overall var by exponentiated
  invlogit <- exp(par) / (1 + exp(par))
  p2r <- p2r_sv.productsum(par, est_object)
  r_s <- r_make(h = est_object$sv$avg_hsp, range = p2r[["s_range"]], structure = est_object$sp_cor)
  r_t <- r_make(h = est_object$sv$avg_tsp, range = p2r[["t_range"]], structure = est_object$t_cor)
